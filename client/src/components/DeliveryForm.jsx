...
export default function DeliveryForm({ total, items = [], orderType = 'delivery', onBack, onSuccess }) {
  ...
  const handlePayment = async (method) => {
    ...
    const deliveryCost = orderType === 'delivery' ? DELIVERY_COST : 0
    try {
      const body = {
        ...form,
        items,
        ...
        telegram_user_id: tgUser?.id ? String(tgUser.id) : undefined
      }
      // For pickup, don't send address details, do send pickup_address
      if (orderType === 'pickup') {
        body.address = null
    ...
    return (
      <div className="success-screen">
        ...
        <div className="success-text">
          ...
          {orderType === 'pickup' ? (
            <>Забрать: <strong>{PICKUP_ADDRESS}</strong></>
          ) : (
  ...
  if (step === 'payment') {
    return (
      <>
        ...
        <div className="payment-methods">
          <label className={`payment-option${form.payment_method === 'cash' ? ' selected' : ''}`}>
            ...
            <div>
              ...
              <div className="payment-desc">Оплата при получении</div>
            </div>
          </label>
        ...
        {orderType === 'pickup' && (
          <div style={{ marginBottom: 16, padding: '10px 12px', backgroundColor: 'rgba(0,168,120,0.08)', borderRadius: 'var(--radius-sm)', fontSize: 13, color: 'var(--text-secondary)' }}>
            📍 Место самовывоза: <strong style={{ color: 'var(--emerald-light)' }}>{PICKUP_ADDRESS}</strong>
          </div>
        )}
  ...
  // Step 1: Delivery/Pickup form
  return (
    <>
      <div className="modal-title">
        {orderType === 'pickup' ? '📍 Самовывоз' : '🚚 Оформление доставки'}
      </div>

      {orderType === 'pickup' && (
        <div style={{ marginBottom: 20, padding: '12px 14px', backgroundColor: 'rgba(0,168,120,0.08)', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border)' }}>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 4 }}>Адрес самовывоза:</div>
          <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--emerald-light)' }}>{PICKUP_ADDRESS}</div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit}>
        ...
        {orderType === 'pickup' && (
          <div className="form-group">
            ...
            <textarea className="form-textarea" placeholder="Пожелания (необязательно)" value={form.comment}
              onChange={e => update('comment', e.target.value)} rows={2} />
          </div>
        )}
        ...
        <button type="submit" className="submit-btn">
          {orderType === 'pickup' ? 'Продолжить к оплате' : 'Продолжить к оплате'}
        </button>
